...
Scenario: The default world
  Given light ← point_light(point -10, 10, -10,color 1, 1, 1)
    And s1 ← new sphere with:
      | material.color     | color(0.8, 1.0, 0.6)   |
      | material.diffuse   | 0.7                    |
      | material.specular  | 0.2                    |
    And s2 ← new sphere with:
      | transform | scaling(0.5, 0.5, 0.5) |
  When w ← new default_world
  Then w.light = light
    And w contains s1
    And w contains s2
...
Scenario: The reflected color for a nonreflective material
  ...
  When comps ← prepare_computations(i, r)
    And col ← reflected_color(w, comps)
...
Scenario: The reflected color for a reflective material
  ...
  When comps ← prepare_computations(i, r)
    And col ← reflected_color(w, comps)
  Then col = color(0.19032, 0.2379, 0.14274)
...
Scenario: shade_hit() with a reflective material
  Given w ← new default_world
    ...
    And i ← intersection(√2, shape)
  When comps ← prepare_computations(i, r)
    And col ← shade_hit(w, comps)
  Then col = color(0.87677, 0.92436, 0.82918)

Scenario: color_at() with mutually reflective surfaces
  Given w ← new world
    And w.light ← point_light(point 0, 0, 0,color 1, 1, 1)
    And lower ← new plane with:
      | material.reflective | 1                     |
      | transform           | translation(0, -1, 0) |
    And lower is added to w
    And upper ← new plane with:
      | material.reflective | 1                    |
      | transform           | translation(0, 1, 0) |
...
Scenario: The refracted color with a refracted ray
  Given w ← new default_world
    ...
    And A has:
      ...
      | material.pattern | new test_pattern |
...
Scenario: shade_hit() with a transparent material
  Given w ← new default_world
    And floor ← new plane with:
      | transform                 | translation(0, -1, 0) |
      | material.transparency     | 0.5                   |
      | material.refractive_index | 1.5                   |
    And floor is added to w
    And ball ← new sphere with:
  ...
  When comps ← prepare_computations(xs[0], r, xs)
    And col ← shade_hit(w, comps, 5)
  Then col = color(0.93642, 0.68642, 0.68642)
...
Scenario: shade_hit() with a reflective, transparent material
  ...
  When comps ← prepare_computations(xs[0], r, xs)
    And col ← shade_hit(w, comps, 5)
  Then col = color(0.93391, 0.69643, 0.69243)
